...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/08/14 12:59:07 by averemiy          #+#    #+#             */
/*   Updated: 2018/08/26 19:59:07 by averemiy         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static unsigned int			*create_to_32(unsigned char *str, int len)
{
	unsigned int			*test;
	int						i;
	int						j;
...
void						hash_sha256(t_alg *alg)
{
	...
	add_length_sha(str, len, len_bit);
	len += 8;
	test = create_to_32(str, len);
	create_hash_sha256(test, len, alg);
	free(str);
	free(test);
}
